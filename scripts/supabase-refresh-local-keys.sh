#!/usr/bin/env bash
# Lit API_URL, DB_URL et SERVICE_ROLE_KEY via `supabase status` → supabase/.env (STORE).
set -euo pipefail
...
if ! command -v supabase &>/dev/null; then
  echo "[sucre-store] CLI supabase absente." >&2
  exit 1
fi
if [[ ! -f "$ENV_FILE" ]]; then
  echo "[sucre-store] Créez d'abord $ENV_FILE (ex. cp supabase/.env.example supabase/.env)" >&2
  exit 1
fi

if ! (cd "$WORKDIR" && supabase status -o env >"$TMP" 2>/dev/null); then
  echo "[sucre-store] Lancez : cd \"$WORKDIR\" && supabase start" >&2
  exit 1
fi
...
[[ -z "$SRK" ]] && {
  echo "[sucre-store] Pas de SERVICE_ROLE_KEY dans supabase status." >&2
  exit 1
}
...
"${ARGS[@]}"
echo "[sucre-store] Mis à jour $ENV_FILE." >&2

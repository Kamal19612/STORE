...
public class FcmService {
    ...
    public void notifyDeliveryAgentsNewDelivery(Order order) {
        ...
        List<DeliveryDeviceToken> tokens = tokenRepository.findByUserRoleAndIsActiveTrue(User.Role.DELIVERY_AGENT);
        if (tokens.isEmpty()) {
            log.warn(
                "[NOTIF] channel=FCM order={} status=SKIPPED error=no_active_delivery_fcm_tokens (register device tokens for ROLE_DELIVERY_AGENT)",
                order.getOrderNumber());
            return;
        }
    ...
    public void notifyAdminsNewOrder(Order order) {
        ...
        tokens.addAll(tokenRepository.findByUserRoleAndIsActiveTrue(User.Role.ADMIN));
        ...
        if (tokens.isEmpty()) {
            log.warn(
                "[NOTIF] channel=FCM order={} event=new_order status=SKIPPED error=no_admin_fcm_tokens (ADMIN/SUPER_ADMIN/MANAGER must register via POST /api/delivery/devices/register)",
                order.getOrderNumber());
            return;
        }
